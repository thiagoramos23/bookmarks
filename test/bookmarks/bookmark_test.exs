defmodule Bookmarks.BookmarkTest do
  ...
  describe "websites"  do
    ...
    @valid_attrs %{is_favorite: true, logo_url: "some logo_url", name: "some name", tags: "some tags", url: "some url"}
    ...
    test "list_websites/1 returns all websites by name or url", %{user: user} do
      ...
      other_website = website_fixture(%{user: user, name: "My Castle", url: "https://twitter.com"})

      assert Bookmark.list_websites_by(%{user_id: user.id, param: "Twi"}) == [website, other_website]
    end
    ...
    test "create_website/1 with valid data creates a website", %{user: user} do
      assert {:ok, %Website{} = website} = Bookmark.create_website(%{user: user} |> Enum.into(@valid_attrs))
    ...
    test "create_website/1 with invalid data returns error changeset", %{user: user} do
      assert {:error, %Ecto.Changeset{}} = Bookmark.create_website(%{user: user} |> Enum.into(@invalid_attrs))
    end

    test "update_website/2 with valid data updates the website", %{user: user} do
      website = website_fixture(%{user: user})
      assert {:ok, %Website{} = website} = Bookmark.update_website(website, %{user: user} |> Enum.into(@update_attrs))
